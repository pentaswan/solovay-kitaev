...
def _group_commutator_decompose(matrix, tol=1e-5):
    ...
    # print("eigenvalues", qml.math.linalg.eig(matrix)[0])
    # print("ud", ud)
    # print("vwd", vwd)

    # print("ud", ud)
    # print("gc", qml.matrix(v @ w @ v.adjoint() @ w.adjoint()))
    # print("vwd", vwd)
    # print("ud", qml.math.linalg.eig(matrix)[0])
    # print("vwd", qml.math.linalg.eig(qml.matrix(v @ w @ v.adjoint() @ w.adjoint()))[0])

    # print("vwd_conj", qml.math.conj(qml.math.transpose(vwd)))
    ...
    **Example**
    ...
    with QueuingManager.stop_recording():
        ...
        for depth in range(max_depth):
            ...
            # print("calling _s_k")
            # print("gate_mat", gate_mat)
            # print("depth+1", (depth + 1))
            # print("decomposition", decomposition)
            # decomp_product = np.array([[1, 0], [1, 0]])
            # for op in decomposition:
            #     decomp_product = decomp_product @ op.matrix()
            # print("decomp product", decomp_product)# qml.prod(ops).matrix()
            # print("u_prime", u_prime)
            # print("perhaps phase", approx_set_gph[index])
            # # print("product (phase)", u_prime @ qml.math.conj(qml.math.transpose(decomp_product)))


            # print("big call", depth)
            # print("decomposition")
            # print(decomposition)
            # print("u_prime")
            # print(u_prime)
...
# # Get the gate decomposition in ['H', 'S', 'T']
# ops = sk_decomposition(op, epsilon=1e-3)
...
# ])
# matrix = qml.QubitUnitary(U, wires=0)
# # Get the gate decomposition in ['H', 'S', 'T']
# ops = sk_decomposition(matrix, epsilon=1e-3, max_depth=3)

# matrix_sk = qml.prod(*reversed(ops)).matrix()

# print("final matrix", matrix_sk)
# # print("final ops", ops)

# U = np.array([
#     [1 / qml.math.sqrt(3), qml.math.sqrt(2 / 3)],
#     [qml.math.sqrt(2 / 3), -1 / qml.math.sqrt(3)]
# ])
# # matrix = qml.QubitUnitary(matrix, wires=0)
# v, w = _group_commutator_decompose(U)

# print("v", v)
# print("w", w)

U = np.array([
    [0.439-0.288j, 0.793-0.309j],
    [-0.742+0.417j, 0.501-0.156j]
])

matrix = qml.QubitUnitary(U, wires=0)
ops = sk_decomposition(matrix, epsilon=1e-3, max_depth=0)
