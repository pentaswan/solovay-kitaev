...
# n = 0 approximation route
@app.route("/approx", methods=["POST"])
def handle_approx():
    data = request.get_json()

    U = np.array([
        [data[0].get("real") + data[0].get("imaginary") * 1j, data[1].get("real") + data[1].get("imaginary") * 1j],
        [data[2].get("real") + data[2].get("imaginary") * 1j, data[3].get("real") + data[3].get("imaginary") * 1j]
    ])

    interface = qml.math.get_deep_interface(U)
    gate_mat, gate_gph = _SU2_transform(qml.math.unwrap(U))
    gate_qat = _quaternion_transform(gate_mat)
    ...
    for row in u_prime:
        for element in row:
            data.append(element.real)
            data.append(element.imag)
...
def handle_json():
    ...
    result = _group_commutator_decompose(U)
...
def handle_eig():
    ...
    eig_matrix = qml.math.linalg.eig(U)[1]

    data = []

    for row in eig_matrix:
        for element in row:
            data.append(element.real)
            data.append(element.imag)
